...
Cuando tenemos un generador de un determinado tipo de dato podemos
hacerlo funcionar mediante

\begin{sesion}
generate :: Gen a -> IO a
\end{sesion}

Veamos algunos ejemplos fáciles de tipos de datos que ya pertenecen a la
clase \texttt{Arbitrary}:
...
Posteriormente añadimos el tipo \texttt{(Arbol a)} a la clase
\texttt{Arbitrary}:
...
\begin{tabular}{|l | l | }
  \hline
  \texttt{choose} & Escoge de manera pseudoaleatoria un elemento de un
                    intervalo \texttt{(a,b)}. \\ \hline
  \texttt{oneof}  & Escoge de manera pseudoaleatoria un elemento de una
                    lista. \\ \hline
  \texttt{listOf} & Elabora una lista de elementos pseudoaleatorios de
                    un tipo determinado. \\ \hline
  \texttt{liftM}  & Convierte una función en una mónada.\\ \hline
  \texttt{liftM2} & Convierte una función en una mónada escaneando
                    los argumentos \\
...
En una futura sección se implementarán generadores de los distintos
tipos de datos que se establecen para las fórmulas de la lógica, y así
poder hacer comprobaciones con QuickCheck de todo aquello que
implementemos.
