...
En esta sección procurarémos dar una implementación alternativa de la sustitución empleando
funciones procedentes de la librería \texttt{Data.Map}.
...
Se define la sustitución de términos mediante la función \texttt{sustTerm t d}, donde
\texttt{t} representa el término que será sustituido y \texttt{d} el diccionario en el
que se definen las distintas sutituciones.
...
Como en una fórmula pueden aparecer varios términos en una misma lista, implementamos
una generalización de la función anterior, \texttt{(sustTerms ts d)}.
...
Finalmente, se define \texttt{(sustitucion f d)} que calcula
